...
public class SalesController {
    ...
    @Operation(
            ...
            description = "Позволяет добавить новую продажу в базу данных"
    )
    @PostMapping("/new")
    public ResponseEntity<SalesDTO> create(@Parameter(description = "Описание новой продажи")
                                           @RequestBody SalesDTO sales) {
        NullFieldCheck.checkFields(sales);
        return ResponseEntity.ok(
                salesService.create(sales)
                        .orElseThrow(() -> new SalesCreateException("Failed to create sales", "unknown"))
        );
    }

    @Operation(
            summary = "Прочитать все продажи",
    ...
    @Operation(
            summary = "Прочитать продажу по id",
    ...
    public ResponseEntity<SalesDTO> getById(@PathVariable Long id) {
        return salesService.getById(id)
                ...
                .orElseThrow(() -> new SalesNotFoundException("Sales with id " + id + " not found"));
    }

    @Operation(
            summary = "Обновить данные о продаже",
    ...
    public ResponseEntity<SalesDTO> update(@PathVariable Long id, @RequestBody SalesDTO updateSales) {
        return salesService.update(id, updateSales)
                .map(ResponseEntity::ok)
                .orElseThrow(() -> new SalesNotFoundException("Sales with id " + id + " not found"));
    }

    @Operation(
            summary = "Получить продажи за определенный период",
    ...
    public ResponseEntity<List<Optional<SalesDTO>>> getByDateRange(@PathVariable String filter) {
        ...
        if (!isPresent) {
            throw new InvalidFilterException(HttpStatus.BAD_REQUEST,
                    "Invalid Filter " + "\"" + filter + "\", " + "Enter one of the filters: week/month/year",
                    new Throwable("Invalid filter value"));
        }
